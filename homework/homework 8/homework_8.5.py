...
class Storage:
    ...
    def transfer(self, idx: int, department: str):
        if not isinstance(idx, int):
            raise TransferStorageError(f"Wrong type '{type(item)}'")

        item: OfficeEquipment = self.__storage[idx]

        if item.department:
            raise TransferStorageError(f"Equipment {item} already link with {item.department}")
    ...
    def __str__(self):
        return f"in storage there are {len(self.__storage)} Equipment"
...
class Xerox(OfficeEquipment):
    def __init__(self, *args):
        super().__init__('Photocopier', *args)


if __name__ == '__main__':
    storage = Storage()
    storage.add(Printer("HP", "XP-800"))
    storage.add(Scanner("Epson", "3762-FG"))
    ...
    print("transfer 1 Equipment")
    storage.transfer(last_idx, 'Storage department')
